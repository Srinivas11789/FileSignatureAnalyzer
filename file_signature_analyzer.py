###############################################################################################
#
#                 Forensics Assigment 2 - File Type Determination Program
#
#       ARGUMENTS:
#               * File          - STRING - Name of the File used
#               * MD5 Verify    - BOOL   - MD5 Hash of the contents before and after read
#
#       DESCRIPTION:
#
#
...
table_of_content = PrettyTable(['FileName', 'Type', 'MagicNumber','RandomChar','Md5BeforeAnalysis', 'Md5AfterAnalysis'])


#for file_name in file_names:
for file_name in os.listdir(os.getcwd()):
    # File Open and Content Read
    # Original HASH Calculation
    original_hash =  hashlib.md5(open(file_name,'rb').read()).hexdigest()

    # File Read for 4 Bytes of Magic Number
    file_handle = open(file_name, 'r')
    first_line  = file_handle.read(4).encode('hex')
    file_handle.close()
    ...
    file_format = []
    # Check for MAGIC Number existence in the First Line
    for key,value in magic_number_intelligence.iteritems():
        for item in value['signs']:
            signs = []
            signs = item.split(",")
            if first_line.lower() in signs[1].lower():
             #   print signs
              #  print first_line
    ...
    # Hash Verification
    verify = "Fail"
    md5_after_analysis = hashlib.md5(open(file_name,'rb').read()).hexdigest()
    if hash_verify == "True":
